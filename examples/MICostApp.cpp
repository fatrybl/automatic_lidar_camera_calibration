/**
 * @file    MICostApp.cpp
 * Evaluate the mutual-information objective of Pandey et al. at a given LiDAR->camera pose
 * (the initial guess of the parameter file), optionally at perturbed poses, without optimising.
...
int main(int argc, char* argv[])
{
    ...
    printf("pose x=%.4f y=%.4f z=%.4f r=%.3f p=%.3f y=%.3f [deg]  MI=%.6f  (normalize_mi=%d, estimator=%d)\n", base(0), base(1),
           base(2), base(3) / deg, base(4) / deg, base(5) / deg, handler->calculateMICost(base), param.normalizeMI,
           param.probabilityEstimatorType);
    for (int i = 2; i + 5 < argc; i += 6) {
        ...
        printf("perturb dyaw=%s dpitch=%s droll=%s dx=%s dy=%s dz=%s  MI=%.6f\n", argv[i], argv[i + 1], argv[i + 2],
               argv[i + 3], argv[i + 4], argv[i + 5], handler->calculateMICost(t));
    }
    return EXIT_SUCCESS;
}
